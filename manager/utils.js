...
const { timestampToDate, timestampToHour, hexToAscii } = require('../ethereum/web3-utils')
...
const { RINKEBY_ROUND_ROBIN_CONTRACT_ADDRESS } = require('../config')
...
async function sendTransaction(rrContract, method, args = []) {
...
function userRegisteredHandler(userId) {
  if (userId) {
    console.log(`${USER_REGISTERED} event - User ${hexToAscii(userId)} created`)
  }
  else {
...
function userDeletedHandler(userId) {
  if (userId){
    console.log(`${USER_DELETED} event - User ${hexToAscii(userId)} deleted`)
  }
  else {
...
function taskCreatedHandler(taskId) {
  if (taskId) {
    console.log(`${TASK_CREATED} event - Task ${hexToAscii(taskId)} created`)
  }
  else {
...
function taskAcceptedHandler(cronJobs, { userId, taskId }) {
  if (taskId && userId) {
    console.log(`${TASK_ACCEPTED} event - Task ${hexToAscii(taskId)} accepted by user ${hexToAscii(userId)}`)
    if (cronJobs.has(taskId)) {
...
function taskDeletedHandler(cronJobs, { taskId }) {
  if (taskId) {
    console.log(`${TASK_DELETED} event - Task ${hexToAscii(taskId)} deleted`)
    if (cronJobs.has(taskId)) {
...
function rejecterDeletedHandler(userId, taskId) {  
  console.log(`${REJECTER_DELETED} event - Task ${hexToAscii(taskId)} rejecter ${hexToAscii(userId)} deleted`)
}
...
function createReallocationCronJob(rrContract, taskId, timestamp) {
  ...
  return new CronJob(executionDate, function () {
    sendTransaction(rrContract, 'reallocateTask', [taskId]).then(
      () => console.log(`Job ${hexToAscii(taskId)} executed on ${executionDate}`)
    )
  })
}
 ...
 */
async function createJobsForAllocatedTasks (cronJobs, rrContract) {
  // Get task ids
...
async function setUpEventListeners(cronJobs, rrContract) {
...
module.exports = { getRRContract, createJobsForAllocatedTasks, setUpEventListeners }